...
Deno.test("isUpdateQuery", async (t) => {
  await t.step("returns false for SELECT query", () => {
    assertEquals(
      isUpdateQuery("SELECT * WHERE { ?s ?p ?o }"),
      false,
    );
  });

  await t.step("returns false for ASK query", () => {
    assertEquals(
      isUpdateQuery("ASK WHERE { ?s ?p ?o }"),
      false,
    );
  });

  await t.step("returns false for CONSTRUCT query", () => {
    assertEquals(
      isUpdateQuery("CONSTRUCT { ?s ?p ?o } WHERE { ?s ?p ?o }"),
      false,
    );
  });

  await t.step("returns false for DESCRIBE query", () => {
    assertEquals(
      isUpdateQuery("DESCRIBE <http://example.org/subject>"),
      false,
    );
  });

  await t.step("returns true for INSERT DATA query", () => {
    assertEquals(
      isUpdateQuery("INSERT DATA { <s> <p> <o> }"),
      true,
    );
  });

  await t.step("returns true for INSERT query", () => {
    assertEquals(
      isUpdateQuery("INSERT { <s> <p> <o> } WHERE { ?x ?y ?z }"),
      true,
    );
  });

  await t.step("returns true for DELETE DATA query", () => {
    assertEquals(
      isUpdateQuery("DELETE DATA { <s> <p> <o> }"),
      true,
    );
  });

  await t.step("returns true for DELETE query", () => {
    assertEquals(
      isUpdateQuery("DELETE { <s> <p> <o> } WHERE { ?x ?y ?z }"),
      true,
    );
  });

  await t.step("returns true for LOAD query", () => {
    assertEquals(
      isUpdateQuery("LOAD <http://example.org/data.ttl>"),
      true,
    );
  });

  await t.step("returns true for CLEAR query", () => {
    assertEquals(
      isUpdateQuery("CLEAR DEFAULT"),
      true,
    );
  });

  await t.step("returns true for DROP query", () => {
    assertEquals(
      isUpdateQuery("DROP GRAPH <http://example.org/graph>"),
      true,
    );
  });

  await t.step("returns true for CREATE query", () => {
    assertEquals(
      isUpdateQuery("CREATE GRAPH <http://example.org/graph>"),
      true,
    );
  });

  await t.step("returns true for ADD query", () => {
    assertEquals(
      isUpdateQuery(
  ...
  await t.step("returns true for MOVE query", () => {
    assertEquals(
      isUpdateQuery(
  ...
  await t.step("returns true for COPY query", () => {
    assertEquals(
      isUpdateQuery(
  ...
  await t.step("handles queries with PREFIX declarations", () => {
    assertEquals(
      isUpdateQuery(`
  ...
  await t.step("handles queries with BASE declaration", () => {
    assertEquals(
      isUpdateQuery(`
  ...
  await t.step("handles queries with comments", () => {
    assertEquals(
      isUpdateQuery(`
        # This is a comment
        INSERT DATA { <s> <p> <o> }
  ...
  await t.step("handles queries with extra whitespace", () => {
    assertEquals(
      isUpdateQuery("   INSERT   DATA   {   <s>   <p>   <o>   }   "),
      true,
    );
  ...
  await t.step("handles queries with newlines", () => {
    assertEquals(
      isUpdateQuery(`
        INSERT
        DATA
  ...
  await t.step("is case insensitive", () => {
    ...
    assertEquals(
      isUpdateQuery("DELETE WHERE { ?s ?p ?o }"),
      true,
    );
  });

  await t.step("returns false when update keyword appears in comment", () => {
    assertEquals(
      isUpdateQuery(
        "# INSERT DATA { <s> <p> <o> } SELECT * WHERE { ?s ?p ?o }",
      ),
      false,
    );
  });
  ...
  await t.step(
    "returns false when update keyword appears in string literal",
    () => {
      assertEquals(
        isUpdateQuery('SELECT * WHERE { ?s ?p "INSERT DATA" }'),
        false,
      );
    },
  );

  await t.step("handles complex SELECT with subqueries", () => {
    assertEquals(
      isUpdateQuery(`
        SELECT ?s ?p ?o WHERE {
          ?s ?p ?o .
  ...
  await t.step("handles empty query", () => {
    assertEquals(isUpdateQuery(""), false);
  });

  await t.step("handles query with only whitespace", () => {
    assertEquals(isUpdateQuery("   \n\t  "), false);
  });
});
